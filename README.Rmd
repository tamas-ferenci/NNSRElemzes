...
Ez `r sum(Veszt2$N2)` eset; ezeket szintén hagyjuk most el (láthatóan ismét minimális veszteség):
...
Ezzel előállítottunk egy tisztított, további feldolgozásra alkalmas adatbázist. Ellenőrizzük, hogy jól dolgoztunk-e ennek során! Bár az adatbázisban már csak `r sum(Res$N)` esetünk van, de a három lépés során összesen `r sum(Veszt1$N1) + sum(Veszt2$N2) + sum(Veszt3$N3)` esetet hagytunk el, így szerencsére megkapjuk a teljes számot, ami `r nrow(RawData)`, ahogy azt már láttuk is. Ellenőrizzük le, hogy a részletesebb lebontások is stimmelnek:
...
Ezzel fertőzéses esetet egyáltalán nem veszítünk, ápolási napot ugyan igen, de még így is megőriztük az ápolási napok nagyon nagy hányadát:
